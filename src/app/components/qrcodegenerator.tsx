  ...
  return (
    <div className="border-slate-200 border rounded-md flex flex-col lg:flex-row text-center  bg-slate-200 text-slate-900 p-4">
      <div className="border bg-slate-200 my-auto " id="qr-code">
        <QrCode value={qrCodeValue} className="p-2 mx-auto bg-white" size={qrCodeSize} title={qrCodeValue} />
      ...
      <div className="flex flex-col text-left lg:pl-4 justify-evenly">
        ...
        <div className="flex flex-col my-2">
          ...
          <select name="size" className="text-center my-2" onChange={(e) => setQrCodeSize(e.target.value)}>
            <option value={128}>128px x 128px</option>
            <option value={256} selected="selected">
              256px x 256px (Default)
            </option>
          </select>
        </div>
        <button className="bg-transparent hover:bg-slate-900 text-slate-900 font-semibold hover:text-slate-200 py-2 my-2 px-4 border border-slate-900 hover:border-transparent rounded" onClick={(e) => handleDownload()}>
          Download
        </button>
      </div>
    </div>
